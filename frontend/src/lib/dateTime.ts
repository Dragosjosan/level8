...
export function requireUtcDateTime(
    value: string,
    field: string,
): Date {
    ...
    if (date === null) {
        throw new TypeError(
            `Invalid UTC datetime received for ${field}`,
        )
    }
...
export function getFixedWeekHour(
    instant: Date,
    anchor: Date,
): number {
...
export function sortWeeklyInfusions<
    T extends { startsAt: Date },
>(
    infusions: T[],
    windowStart: Date,
): T[] {
